...
const FormationProgressBar = ({ steps, currentStep }) => {
    return (
        <ol className="flex items-center w-full">
            {steps.map((step, index) => (
                <li key={index} className={`flex items-center ${index < steps.length - 1 ? 'w-full' : ''} ${index < currentStep ? 'text-blue-600' : 'text-gray-500'} ${index < steps.length - 1 ? (index < currentStep ? 'after:border-blue-100 dark:after:border-blue-800' : 'after:border-gray-100 dark:after:border-gray-700') : ''} ${index < steps.length - 1 ? 'after:content-[""] after:w-full after:h-1 after:border-b after:border-4 after:inline-block' : ''}`}>
...
export default FormationProgressBar;
